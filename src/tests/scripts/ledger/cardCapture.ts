 ...
 * Usage:
 *   npx ts-node src/tests/scripts/ledger/cardCapture.ts <TOKEN> <WALLET_ID>
 */
async function main() {
    const [token, walletId] = process.argv.slice(2);

    if (!token || !walletId) {
        console.error("Usage: ts-node src/tests/scripts/ledger/cardCapture.ts <TOKEN> <WALLET_ID>");
        process.exit(1);
    }
