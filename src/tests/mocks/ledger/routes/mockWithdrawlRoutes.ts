...
router.post("/:walletId", authMiddleware, async (req, res) => {
  try {
    ...
    const transactionId = `mock_withdraw_${walletId}_${userId}_${Date.now()}`;
    ...
    console.error("Mock withdrawal error:", err);
    return res.status(400).json({
      error: err.message || "Mock withdrawal failed"
    });
  }
});

export { router as mockWithdrawalRoutes };
